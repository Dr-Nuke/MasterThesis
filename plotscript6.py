...
if 1 in plotcase:
    ...
    if 1.5 in subcase: #relative linpow diff
        for iii in [0,2,4,6,8]:
            ...
            for i in range(len(ii)):
                ...
                ph[2],=step(z_mesh,Stepper((LinPow(tp,6,ii[i])-LinPow(tp,5,ii[i]))/LinPow(tp,5,ii[i]),0),ls=ls1[i],color='r',where='post')
                ph[3],=step(z_mesh,Stepper((LinPow(tp,6,ii[i])-LinPow(tp,4,ii[i]))/LinPow(tp,4,ii[i]),0),ls=ls1[i],color='c',where='post')
            ...
            axis.set_xlabel('axial position, [m]',size=14) 
            axis.annotate('{} EFPD'.format((1+iii)*50), xy=(1.5, 0),size=16,color=[0.5,0.5,0.5]) 
    ...
    if 2 in subcase:
        ...
        for i in range(9):
            c = cm.jet((i)/10,1)
            ph[i],=step(z_mesh,Stepper(LinPow(tp,C,i),0)/1000,color=c,where='post',label='{} EFPD'.format((i+1)*50))
        axis.yaxis.grid(True,'major',linewidth=1,color=[0.7,0.7,0.7])
        axis.xaxis.grid(True,'major',linewidth=1,color=[0.7,0.7,0.7])
        axis.set_ylabel('linear Power, [kW/m]' ,size=14)
        axis.set_xlabel('axial position, [m]',size=14) 
