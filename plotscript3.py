...
if 1 in plotcase:
    if 1 in subcase: #power density
        ...
        for i in range(len(ii)):
            ...
            ph[i][2],=step(z_mesh,Stepper(LinPow(tp,3,ii[i]),0)/1000*3*4,lw=2,color='r',where='post')
            
            
            a=max(abs((LinPow(tp,3,ii[i])-LinPow(tp,0,ii[i]))/LinPow(tp,0,ii[i])))
            b=max(abs((LinPow(tp,3,ii[i])-LinPow(tp,1,ii[i]))/LinPow(tp,1,ii[i])))
        ...
        l=legend([ph[0][0],ph[0][1],ph[0][2]],["case 0","case 1","case 3"],loc=8)
        axis.set_xlim([0,3.6576])
        savefig('V:\\master report\\figures\\powercycle3.pdf')
        axis.set_title('Linear Power development with burnup \n case 3, with 0 and 1',size=18)    
        tight_layout()
        savefig('D:\\dropbox\\Dropbox\\plots\\case3\\powercycle.png')
        show()
...
if 3 in plotcase:        
    ...
    if 1 in subcase:    # plutonium buildup over cycle
        ...
        for i in range(10):
            c = cm.jet((i)/10,1)
            lh[i],=step(z_mesh,Stepper(PuDens(tpu[3,i,:,:,:,0],3),0),color=c,where='post')
            step(z_mesh,Stepper(PuDens(tpu[3,i,:,:,:,1],3),0),color=c,where='post')        
    ...
    if 1.5 in subcase:
        ...
        for i in range(len(case)):
            ...
            lh[i][0],=step(z_mesh,Stepper(PuDens(tpu[3,case[i],:,:,:,0],3),0),color=c,where='post')
            lh[i][1],=step(z_mesh,Stepper(PuDens(tpu[0,case[i],:,:,:,0],30),0),ls=':',color=c,where='post')        
            
            dif=(PuDens(tpu[3,case[i],:,:,:,0],3)-PuDens(tpu[0,case[i],:,:,:,0],30))/PuDens(tpu[0,case[i],:,:,:,0],30)
        ...
        axis.set_xlabel('axial position, [m]',size=14) 
        #legend()    
        l=legend([lh[1][0],lh[0][0],lh[1][1],lh[0][1]],["case 3","case 3","case 0","case 0"],loc=5)    
        ...
        savefig('V:\\master report\\figures\\Pu_buildup3_1_5.pdf')
        axis.set_title('Pu 239 buildup at 50 & 500 EFPD\n case 3 and 0',size=18)
        tight_layout()
        savefig('D:\dropbox\Dropbox\plots\case3\Pu_buildup1.5.png')            
        show()
